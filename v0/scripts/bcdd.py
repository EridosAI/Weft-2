...
def load_predictor(
    ...
    """
    # inner_pam.py uses absolute imports (from v0.src.config import ...), so
    # the repo root — parent of v0/ — must be on sys.path.
    # predictor_module_path = <repo_root>/v0/src/predictor/inner_pam.py
    repo_root = predictor_module_path.resolve().parents[3]
    if str(repo_root) not in sys.path:
        sys.path.insert(0, str(repo_root))

    from v0.src.predictor.inner_pam import InnerPAM  # type: ignore[import-not-found]
...
def main():
    ...
    parser.add_argument("--embeddings", type=Path,
        default=Path("v0/data/phase2_embeddings/embeddings.npy"),
        help="Path to Phase 2 DINOv2 embeddings .npy (N, 1024)")
    parser.add_argument("--predictor-module", type=Path,
        default=Path("v0/src/predictor/inner_pam.py"),
