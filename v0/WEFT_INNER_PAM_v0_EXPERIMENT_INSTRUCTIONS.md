...
- **Loop length:** **360 frames per loop** empirically at the v2 substrate (5-loop calibration, 2026-05-14: 1,800 frames / 5 = 360 frames per loop, comprising ~55 close-up + ~305 transit + corner-rotation frames). The +13.9% increase from the v1 substrate's 316 frames/loop is structural to the DT pose change — the new DT position requires longer NavMesh-planned transits to/from Bed and Dresser. Loop length increase accepted on downstream-arithmetic grounds rather than the round-number 10% SCAFFOLDING threshold (see HANDOFF session-5 fourth-STOP entry and the rationale in §8.3's margin discussion). v3 calibration (post-floor-y-fix, same DT pose) reproduces 360 frames/loop within ±5%, confirming the loop-length value is robust to the y-fix.
- **Agent camera height** is set by `ContinuousMotionExplorer` at controller-init time, NOT inherited from `route_phase2.json`'s `viewing_position.y` values. The explorer queries `controller.step("GetReachablePositions")` once and stores the **modal y across all reachable positions** on `self._agent_floor_y`; every subsequent `Teleport` call passes that fixed y regardless of what the input position dict carries. The modal-y method (per spec §5.7) is robust to scenes with stairs or sloped floors — it picks the dominant floor level the route's items live on. The full y-distribution summary is captured on `self.floor_y_summary` for HANDOFF audit. The 2026-05-14 sixth-STOP trajectory diagnostic showed the original code copied y from two different sources by phase (route's `viewing_position.y ≈ 0.901` for close-up steps, NavMesh waypoint `y ≈ 0.006` for transit steps), producing a 0.894 m camera-elevation oscillation at every phase boundary; the fix and the explicit floor-y derivation are recorded in spec §5.7.
...
`scripts/run_phase2_encode.py` runs frozen DINOv2 over Phase 2 frames, **writes `data/phase2_embeddings/embeddings.npy` unconditionally** (save-first / gate-second; session-6 directive 2026-05-14 so the statistical gate can be re-run cheaply on the saved matrix without re-encoding 65k frames), then applies the §8.4 verification suite. **Embeddings on disk do NOT imply gate-PASS**; the encode_report.json and the script's exit code are the load-bearing signals. Launch training only when `overall_pass == true` in the report and the script exits zero.
...
2. **Ratio gate (clean-control formulation; restructured 2026-05-14 per session-6 reviewer authorisation).** For each item, compute a per-stage mean-cosine "gap": draw 25 Stage A apex frames and 25 Stage B apex frames at the item's `viewing_position_id`, compute `within_a`, `within_b`, and the all-pairs `cross_stage` mean cosine, take `gap = 0.5 * (within_a + within_b) − cross_stage`. Aggregate:
   - `perturbed_mean_gap = mean(gap on Dresser, Sofa)`
   - `clean_control_mean_gap = mean(gap on Bed, Television)`
   - `gap_DiningTable` reported as a record-only noisy-control diagnostic.

   Gate passes iff `perturbed_mean_gap / clean_control_mean_gap ≥ 2.0`, or `clean_control_mean_gap ≤ 1e-3` (locality clean by construction; then gate passes iff `perturbed_mean_gap` clears the same `1e-3` near-zero floor).

   *Rationale.* DiningTable's h118-corrected pose still leaks residual doorway-bleed; its gap (~0.014 at session-5 collection) sits in the same band as the perturbed items' gaps and contaminates the control-mean if averaged in. The {Bed, Television} clean-control set is the third-STOP framing the experiment chat authorised. The 0.05 absolute floor that gated this check pre-session-6 was a pre-empirical SCAFFOLDING guess inherited from spec §5.1/5.2's pair-cosine pattern (gross scene-level contrasts at ~0.5 magnitude), wrong-scale for the within-item perturbation responses (~0.01 magnitude) `RandomizeMaterials` actually produces. Dropped per session-6 reviewer authorisation and per `research_operations_v1.md` §15's principle on absolute-magnitude thresholds.

3. **Wilcoxon signed-rank gate (statistical-distinguishability; new 2026-05-14 per session-6 Reading C).** For each perturbed item, build the per-pair `(1 − cross_stage_cosine)` distribution from the full Stage A apex × Stage B apex grid (n_a × n_b pairs ≈ 31 × 150 = 4650 values), then run `scipy.stats.wilcoxon(diffs, alternative='greater', zero_method='wilcox', method='approx')`. This tests whether the median of `(1 − cos)` is significantly greater than zero — i.e. whether the cross-stage cosines sit systematically below 1.0.

   Bonferroni-correct for two perturbed items: `corrected_p = min(raw_p * 2, 1.0)`. Gate passes iff `corrected_p < 0.001` on each of Dresser and Sofa (equivalent to `raw_p < 5e-4` per item). The same test is run on Bed, Television, and DiningTable as a record-only sanity diagnostic (not gated); their corrected p-values capture how much Stage A → Stage B drift the encoder produces on items the perturbation does not visually re-texture.

   *Rationale.* The architecturally-meaningful question is "is the perturbation producing a signal the predictor can learn from?", which is a distributional-distinguishability question rather than an absolute-magnitude question. The signed-rank formulation on `(1 − cos)` tests directly whether cross-stage cosines are systematically below the within-stage ceiling of 1.0, without anchoring on any pre-empirical magnitude. Per `research_operations_v1.md` §15: "When a SCAFFOLDING threshold has the form 'absolute magnitude ≥ K' for an architectural property where K was guessed before empirical data on the substrate existed, the threshold is structurally vulnerable to scale mismatch. Prefer: (a) statistical-distinguishability tests over absolute-magnitude thresholds […]".

A trip on (2) indicates the perturbation produced signal but the perturbed-vs-clean-control separation doesn't meet the ratio criterion. A trip on (3) indicates the perturbation is not statistically distinguishable from "Stage A and Stage B come from the same distribution" — re-collection with a stronger perturbation mechanism (option (iii) per the seventh-STOP framing) would be required. Either failure stops Phase 2 training launch and is reported for experiment-chat review.
...
| SCAFFOLDING | §8.4 clean-control set = {Bed, Television}; DT recorded as noisy-control diagnostic | this doc §8.4 | revisit if a substrate revision removes DT's residual LivingRoom doorway-bleed |
