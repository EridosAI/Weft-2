...
Next sub-phase: **0.6 — V2-PRE-E** (SCAFFOLDING calibration, analytical, 0 arm-runs): calibrate τ_W per-head (Wilcoxon p<0.05 vs the D1a bit-identical baseline), τ_L/τ_R/τ_pert (vs PRE-A distributions), repetition-coverage threshold (PRE-B coverage=1.0 must clear it), BIC threshold (PRE-B flagged placeholder 10 too permissive), local-PCA window. V2_TRAINING_STEPS gate cleared (10000 adequate). Then 0.7 PRE-D2 (200 arm-runs, ~7hr), Phase 0 HANDOFF.
